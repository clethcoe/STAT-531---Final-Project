...
As reported by the [New York Times](https://www.nytimes.com/2023/11/06/health/tuberculosis-tb-treatment-vaccine-diagnosis.html), tuberculosis (TB) is again the world's leading infectious disease killer as of November 2023, after being briefly surpassed by COVID-19. The World Health Organization reports that nearly 40 percent of people who are living with TB are untreated and undiagnosed. Despite being a preventable and treatable disease, 1.36 million people died from tuberculosis in 2022.

This project aims to better contextualize the relationship between government healthcare spending and tuberculosis on a country-by-country basis. We hypothesize that the countries that spend more on healthcare per person also see fewer deaths from tuberculosis.
...
The data sets were joined by country and only complete observations from 2000 - 2010 were included. The TB deaths data included 216 countries while the government healthcare spending data included 192 countries. Our combined dataset includes 192 countries.
...
To examine how the relationship between our two variables of interest changed over time, we created an animated plot that shows deaths per 100k vs government healthcare spending per person in USD for every country in every year of the study (2000 - 2010).
...
Over time, the amount of government spending per person increased on average. The relationship between deaths per 100k and government spending displays a hyperbolic relationship over time, suggesting that as government spending increases, the number of deaths per 100k decreases. To confirm this inference, we created density plots of deaths per 100k and government healthcare spending per person to show how the proportional distribution changes over time. 
...
From these plots, it becomes clearer that as government healthcare spending increased over time, the number of deaths per 100k decreased.
...
combined_dataset_long_cleaned |> 
  ...
  geom_smooth(method = "lm",
              color = "black") + 
  xlab("Mean Government Spending per Person, USD") + 
  ylab("") + 
  ggtitle("Mean TB Deaths per 100k")+
  theme_bw()
```
...
Average TB deaths per 100,000 = 44.52 - 0.017(Average government healthcare spending per person, USD)

#### Interpretation

There is a negative correlation between average TB deaths per 100k and average government healthcare spending per person. Our model suggests that for every \$1000(USD) spent by a government on healthcare, on average, 17 people per 100,000 are prevented from dying of TB. It also suggests that if a government does not spend anything per person on healthcare, on average, 44 people out of 100,000 will die of TB per year. However, the relationship between government healthcare spending and TB deaths is not linear and is more similar to a hyperbolic relationship. There is a large amount of variation in average number of TB deaths between countries that spend very little per person on healthcare, and many countries see very few deaths due to TB despite spending very much or little per person on healthcare. Therefore, as a violation of one of the conditions (a linear relationship between *x* and *y*), we would not lean heavily on the results of this linear regression. 

With an R^2^ value of about 0.051, our suspicions from the regression visualization are confirmed and our linear model is not a great fit for the data. Though our model is a poor predictor of the data trends, we can tell from the P-values (both \< 0.001) that there is a very strong correlation between these government healthcare spending and TB deaths.

Additionally, the variance of the fitted values only explain about 6\% of the variability of the response variable (below). The average number of deaths due to TB is only explained in part by government healthcare spending, and the remaining 94\% is unaccounted for in this model. These other factors affecting average TB deaths may include the cost and accessibility of TB screening and drugs, the ratio of healthcare workers to citizens, how often TB drug regimens can be completed, the rate of TB transmission, and many more.  
...
broom::augment(combined_dataset_lm) |> 
  ...
  rename("Variance of average deaths per 100k" = av_deaths_per_100k,
         "Variance of fitted values" = .fitted,
...
The R^2^ values for the simulated and actual data are extremely low. Across 1000 iterations of simulated data, the variability in predicted values explained zero to six percent of the variation in the actual values for TB deaths per 100,000 people. This further confirms that a linear regression is not appropriate for these data.
